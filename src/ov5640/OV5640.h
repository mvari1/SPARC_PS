...
namespace OV5640_cfg {
	...
	using mode_t = enum { MODE_480P_640_480_5FPS = 0, MODE_720P_1280_720_60fps, MODE_1080P_1920_1080_15fps,
	...
	config_word_t const cfg_480p_5fps_[] =
	{// 640 x 480 binned, RAW10, MIPISCLK≈40-100M, SCLK≈10-20MHz, PCLK≈10-20MHz
		//PLL1 configuration
		//[7:4]=0010 System clock divider /8, [3:0]=0001 Scale divider for MIPI /1
		// {0x3035, 0x81},
		...
		{0x3035, 0x21}, // system clk /2, MIPI /1
		{0x3036, 0x46}, // PLL multiplier = 70
		{0x3037, 0x03}, // pre-divider = 1
		...
		// End addresses (readout window size before binning/subsample)
		...
		{0x3807, 1947 & 0xFF}, // 1947 (or 0x079F=1951 full)
		...
		//{0x4837, 0x22},  // Start here (for ~45–56 MHz effective; increase to 0x30+ if PCLK <20 MHz)

		{0x4837, 0x16},  // good for ~48–56 MHz PCLK
        ...
        // Find some way to test that its at 480p@~5fps
    };
...
config_word_t const cfg_1080p_30fps_336M_1lane_mipi_[] =
	...
	{
			{ MAP_ENUM_TO_CFG(MODE_480P_640_480_5FPS, cfg_480p_5fps_) },
